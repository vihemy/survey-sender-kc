...
def check_if_cell_contains_inappropriate_characters(cell):
    return bool(re.search(r'[^0-9+]', cell.value)) # checks if cell value contains anything but numbers and '+'
...
def import_numbers_from_excel_as_list(excel_file, first_row, first_col):
    ...
    for col in range(first_col, sheet.max_column+1):
        for row in range(first_row, sheet.max_row+1):
            ...
            if check_if_cell_content_is_valid(cell): # if cell.value is not empty and does not contains letters, add as phoenumber-object
